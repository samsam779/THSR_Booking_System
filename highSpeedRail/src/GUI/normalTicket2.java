...
public class normalTicket2 {
	...
	public normalTicket2(String START, String DESTINATION, String CARTYPE, String SEATTYPE, String TICKETTYPE,
			String uid) {
		this.START = START;
		...
		/*
		 * 建立一個名為"一般購票"的JFrame(來回購票) 內有兩個JButton 六個JComboBox 四個JTextField
		 */
		p0 = new JPanel();
										...
										if (int2 > int4) {
											...
											if (t1 >= t2) {
												...
												if (diff > 0 && dayDiff < 28) {//如果日期不是未來28天內則不予訂票
													try {
														testticket t = new testticket(START, DESTINATION, CARTYPE,
																SEATTYPE, TICKETTYPE, uid, normalN, elderN, disabledN,
																kidN, goDate, backDate);
											...
											if (diff > 0 && dayDiff < 28) {
												try {
													testticket t = new testticket(START, DESTINATION, CARTYPE, SEATTYPE,
															TICKETTYPE, uid, normalN, elderN, disabledN, kidN, goDate,
															backDate);
												} catch (ParseException e) {
													// TODO Auto-generated catch block
													e.printStackTrace();
